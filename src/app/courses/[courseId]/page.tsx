...
export default function CourseDetailPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  }, [courseId, selectedLesson, course])
  ...
  return <AuthShell active="My learning"><div className="page-wrap detail-page">
    ...
    {loading ? <div style={{ padding: 40, opacity: .4 }}>Loading course...</div> : !course ? <div role="alert" style={{ padding: 40 }}>{loadError || 'Unable to load course.'}</div> : <>
      <div className="detail-hero"><div><span className={`tag tag-${tones[0]}`}>{course.section?.name ?? 'Course'}</span><h1>{course.title}</h1><p>{course.description} Learn through structured modules and focused video lessons.</p><div className="detail-meta"><span><Clock3 size={16} /> {formatDuration(totalMins * 60)}</span><span><Video size={16} /> {lessons.length} {lessons.length === 1 ? 'Lesson' : 'Lessons'}</span><span><ShieldCheck size={16} /> Designed for schools</span>{progressPercent > 0 && <span><CheckCircle2 size={16} /> {progressPercent}% complete</span>}</div></div><div className={`detail-art ${tones[0]}`}><div className="art-shape"><BookOpen size={56} /></div><span>ATLAS<br />LEARNING</span></div></div>
      <div className="lesson-layout"><div>{hasPlaybackId ? <VideoPlayer key={`${courseId}-${selectedLesson?.id ?? 'course'}-${activePlaybackId}`} playbackId={activePlaybackId} courseId={courseId} lessonId={selectedLesson?.id} initialPosition={selectedLesson ? (lessonProgress?.watchedSeconds ?? 0) : (courseProgress?.watchedSeconds ?? 0)} onProgress={handleProgress} /> : <div className="video-stage"><div className="video-placeholder"><div className="play-large"><Play size={26} fill="currentColor" /></div><span>{selectedLesson?.title ?? 'No video connected'}</span><small>{selectedLesson ? 'Mux video is not connected for this lesson yet.' : 'This course has no video yet.'}</small></div></div>}
        ...
        <aside className="lesson-list"><p className="eyebrow">Course content</p><h3>{lessons.length} {lessons.length === 1 ? 'Lesson' : 'Lessons'} · {formatDuration(totalMins * 60)}</h3>{course.modules?.map((module) => <div key={module.id}><p style={{ fontSize: 11, fontWeight: 700, color: 'var(--navy)', margin: '16px 0 4px' }}>{module.title}</p>{module.lessons.map((lesson, index) => <LessonButton key={lesson.id} lesson={lesson} index={index} selected={lesson.id === selectedLesson?.id} onClick={() => setSelectedLessonId(lesson.id)} />)}</div>)}{lessons.length === 0 && <p className="muted" style={{ fontSize: 12 }}>This course has no lessons yet.</p>}</aside>
      </div>
    </>}
  </div></AuthShell>
}
