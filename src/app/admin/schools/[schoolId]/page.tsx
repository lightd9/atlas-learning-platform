...
import { ArrowLeft, Ban, Building2, CheckCircle2, CheckSquare, FileUp, Mail, Pencil, Plus, RefreshCw, School as SchoolIcon, Trash2, UserPlus, Users } from 'lucide-react'
...
export default function AdminSchoolDetailPage({ params }: { params: Promise<{ schoolId: string }> }) {
  ...
  const profileBaseline = { name: school?.name ?? '', slug: school?.slug ?? '', headteacherId: school?.users.find((user) => user.role === 'HEADTEACHER')?.id ?? '' }
  const profileDirty =
    profileForm.name !== profileBaseline.name || profileForm.slug !== profileBaseline.slug || profileForm.headteacherId !== profileBaseline.headteacherId
  ...
  return <AdminShell active="schools">
    <div className="page-wrap">
      ...
      <div className="page-heading compact">
        ...
        <div className="school-profile-actions" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><button className="secondary-button" onClick={() => setEditingProfile(true)}><Pencil size={16} /> Edit profile</button><button className="secondary-button" style={school.active ? { color: '#b42318', borderColor: '#fecdca', background: '#fff' } : undefined} onClick={async () => { const response = await fetch(`/api/admin/schools/${schoolId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ active: !school.active }) }); toast(response.ok ? (school.active ? 'School disabled' : 'School enabled') : 'Unable to update school status', response.ok ? 'success' : 'error'); if (response.ok) load() }}>{school.active ? 'Disable school' : 'Enable school'}</button></div>
      </div>
      ...
      {editingProfile && (
        <AppModal
          ...
          description="The headteacher created with this school is selected by default. You can transfer the role to another teacher from this school."
          icon={<SchoolIcon size={18} />}
          width={640}
          dirty={profileDirty}
          onClose={() => { setEditingProfile(false); setError(''); load() }}
          footer={
            <>
              <button className="secondary-button" onClick={() => { setEditingProfile(false); setError(''); load() }}>Cancel</button>
              <button className="primary-button" type="submit" form="school-profile-form" disabled={profileSaving}>{profileSaving ? 'Saving...' : 'Save profile'}</button>
            </>
          }
        >
          <form id="school-profile-form" onSubmit={saveProfile}>
            <div className="school-profile-fields" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
              <label style={labelStyle}>School name<input required value={profileForm.name} onChange={(e) => setProfileForm({ ...profileForm, name: e.target.value })} style={inputStyle} /></label>
              <label style={labelStyle}>School slug<input required pattern="[a-z0-9-]+" value={profileForm.slug} onChange={(e) => setProfileForm({ ...profileForm, slug: e.target.value.toLowerCase() })} style={inputStyle} /><small className="muted">Lowercase letters, numbers and hyphens.</small></label>
              <label style={labelStyle}>Headteacher<select value={profileForm.headteacherId} onChange={(e) => setProfileForm({ ...profileForm, headteacherId: e.target.value })} style={inputStyle}><option value="">No headteacher assigned</option>{headteachers.map((user) => <option key={user.id} value={user.id}>{user.name}{user.role === 'HEADTEACHER' ? ' (current headteacher)' : ' (teacher)'} — {user.email}</option>)}</select></label>
            </div>
            {error && <p role="alert" style={{ color: '#b42318', fontSize: 12 }}>{error}</p>}
          </form>
        </AppModal>
      )}
      ...
      <section className="panel" style={{ marginBottom: 24 }}>
        ...
        {showInvite && (
          <AppModal
            title="Add teacher"
            eyebrow="School team"
            description={`Send an invitation so a new teacher can join ${school.name}.`}
            icon={<UserPlus size={18} />}
            width={520}
            dirty={form.name.trim() !== '' || form.email.trim() !== ''}
            onClose={() => { setShowInvite(false); setForm({ name: '', email: '' }); setError('') }}
            footer={
              <>
                <button className="secondary-button" onClick={() => { setShowInvite(false); setForm({ name: '', email: '' }); setError('') }}>Cancel</button>
                <button className="primary-button" type="submit" form="add-teacher-form">Send invite</button>
              </>
            }
          >
            <form id="add-teacher-form" onSubmit={inviteTeacher} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <label style={labelStyle}>Teacher name<input aria-label="Teacher name" placeholder="Full name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required style={{ ...inputStyle, maxWidth: '100%' }} /></label>
              <label style={labelStyle}>Email address<input aria-label="Teacher email" type="email" placeholder="Email address" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required style={{ ...inputStyle, maxWidth: '100%' }} /></label>
              {error && <p role="alert" style={{ color: '#b42318', fontSize: 12 }}>{error}</p>}
            </form>
          </AppModal>
        )}
        <div className="table-wrap"><table className="admin-table"><thead><tr>{teamSelectMode && <th style={{ width: 36 }}><input type="checkbox" checked={selectedUserIds.length === school.users.filter((u) => u.id !== session?.user?.id).length && school.users.length > 0} onChange={(e) => setSelectedUserIds(e.target.checked ? school.users.filter((u) => u.id !== session?.user?.id).map((u) => u.id) : [])} aria-label="Select all team members" /></th>}<th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Last active</th><th /></tr></thead><tbody>{school.users.map((user) => <tr key={user.id}>{teamSelectMode && <td>{user.id === session?.user?.id ? null : <input type="checkbox" checked={selectedUserIds.includes(user.id)} onChange={(e) => setSelectedUserIds(e.target.checked ? [...selectedUserIds, user.id] : selectedUserIds.filter((id) => id !== user.id))} aria-label={`Select ${user.name}`} />}</td>}<td><strong>{user.name}</strong></td><td>{user.email}</td><td>{user.role === 'HEADTEACHER' ? 'Headteacher' : user.role === 'INSTRUCTOR' ? 'Instructor' : user.role === 'ATLAS_ADMIN' ? 'Atlas Admin' : 'Teacher'}</td><td><span className={`badge ${user.status === 'ACTIVE' ? 'badge-active' : 'badge-inactive'}`}>{user.status}</span></td><td>{user.lastActiveAt ? new Date(user.lastActiveAt).toLocaleDateString('en-GB') : '—'}</td><td>{user.id === session?.user?.id ? <span className="muted" style={{ fontSize: 12 }}>You</span> : <span style={{ display: 'inline-flex', gap: 4, alignItems: 'center' }}>{session?.user?.role === 'ATLAS_ADMIN' && user.role === 'TEACHER' && <button className="text-button" onClick={() => resetTeacherPassword(user)} style={iconButtonStyle} title={`Reset password for ${user.name}`}></button>}<button className="text-button" onClick={() => deleteMembers([user.id], [])} style={{ ...iconButtonStyle, color: '#b42318' }} title={`Delete ${user.name}`}><Trash2 size={14} /></button></span>}</td></tr>)}</tbody></table></div>
...
const inputStyle: React.CSSProperties = { height: 40, borderRadius: 8, border: '1px solid var(--line)', padding: '0 12px', fontSize: 13, background: '#fff', width: '100%', maxWidth: 300 }
