...
export default function AdminCoursePreviewPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  return <AdminShell active="courses"><div className="page-wrap detail-page">
    <button className="text-button" onClick={() => router.push(`/admin/courses/${courseId}`)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 16 }}><ArrowLeft size={16} /> Back to editor</button>
    ...
    <div className="lesson-layout"><div>{selected?.muxPlaybackId ? <VideoPlayer playbackId={selected.muxPlaybackId} courseId={courseId} lessonId={selected.id} /> : <div className="video-stage"><div className="video-placeholder"><div className="play-large"><Play size={26} fill="currentColor" /></div><span>{selected?.title ?? 'No lesson selected'}</span><small>{selected ? 'Add a Mux playback ID to preview this video.' : 'Add modules and lessons in the editor.'}</small></div></div>}<div className="panel" style={{ marginTop: 16 }}><p className="eyebrow">About this course</p><h2>{course.title}</h2><p className="muted">This is the learner-facing preview. Course progress and completion are disabled in this view.</p><div className="detail-meta"><span><Clock3 size={16} /> {course.durationMinutes} minutes</span><span><Video size={16} /> {modules.reduce((count, module) => count + module.lessons.length, 0)} lessons</span><span><BookOpen size={16} /> {modules.length} modules</span></div></div></div><aside className="lesson-list"><p className="eyebrow">Course content</p>{modules.map((module) => <div key={module.id}><p style={{ fontSize: 11, fontWeight: 700, color: 'var(--navy)', margin: '16px 0 4px' }}>{module.title}</p>{module.lessons.map((lesson) => <button key={lesson.id} className={`lesson ${selected?.id === lesson.id ? 'current' : ''}`} onClick={() => setSelected(lesson)}><span className="lesson-number">{lesson.sortOrder + 1}</span><span>{lesson.title}<small>{Math.round(lesson.durationSeconds / 60)} min</small></span></button>)}</div>)}{modules.length === 0 && <p className="muted">No modules yet.</p>}</aside></div>
  </div></AdminShell>
}
