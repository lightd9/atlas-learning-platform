...
export default function AdminCourseEditPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  function removeModule(moduleId: string) { if (!window.confirm('Delete this module and all its lessons? This cannot be undone.')) return; setModules(modules.filter((module) => module.id !== moduleId).map((module, index) => ({ ...module, sortOrder: index }))) }
  function removeLesson(moduleId: string, lessonId: string) { if (!window.confirm('Delete this lesson? This cannot be undone.')) return; setModules(modules.map((module) => module.id === moduleId ? { ...module, lessons: module.lessons.filter((lesson) => lesson.id !== lessonId).map((lesson, index) => ({ ...lesson, sortOrder: index })) } : module)) }
