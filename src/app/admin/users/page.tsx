...
import { Users, Search, Plus, X, Mail, Power, ShieldCheck, Trash2, KeyRound, Copy, CheckCheck } from 'lucide-react'
...
export default function AdminUsersPage() {
  ...
  const [createForm, setCreateForm] = useState({ name: '', email: '', role: 'INSTRUCTOR', schoolId: '', permissions: [] as string[] })
  ...
  async function resetPassword(user: AdminUser) {
    if (!window.confirm(`Reset the password for ${user.name} (${user.email})? They will be forced to set a new password on next login.`)) return
    const res = await fetch(`/api/admin/users/${user.id}/reset-password`, { method: 'POST' })
    const data = await res.json()
    if (!res.ok || !data.temporaryPassword) { toast(data.error ?? 'Unable to reset password', 'error'); return }
    setCopied(false)
  ...
  return <AdminShell active="users">
    <div className="page-wrap">
      ...
      {resetResult && <div style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, zIndex: 50 }}>
        <div className="panel" style={{ width: '100%', maxWidth: 440, padding: 24 }}>
          <div className="panel-head"><div><p className="eyebrow">Password reset</p><h3>Temporary password for {resetResult.name}</h3></div><button className="icon-button" onClick={() => setResetResult(null)} aria-label="Close"><X size={17} /></button></div>
          <p className="muted" style={{ fontSize: 13, lineHeight: 1.5 }}>Share this password with the user now. It is shown only once. They will be asked to set a new password on their next login.</p>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', margin: '16px 0' }}>
            <code style={{ flex: 1, padding: '10px 12px', fontSize: 14, fontWeight: 700, background: '#f7f8fb', border: '1px solid var(--line)', borderRadius: 8, wordBreak: 'break-all' }}>{resetResult.password}</code>
            <button className="secondary-button" onClick={copyTemporaryPassword} style={{ whiteSpace: 'nowrap' }}>{copied ? <CheckCheck size={15} /> : <Copy size={15} />}{copied ? ' Copied' : ' Copy'}</button>
          </div>
          <button className="primary-button" onClick={() => setResetResult(null)} style={{ width: '100%' }}>Done</button>
        </div>
      </div>}
