...
import { Users, Search, Plus, X, Ban, CheckCircle2 } from 'lucide-react'
...
export default function AdminUsersPage() {
  ...
  async function createUser(e: React.FormEvent) {
    e.preventDefault(); setCreateError(''); setSetupUrl('')
            ...
            <tbody>
              {filtered.map((user) => (
                <tr key={user.id}>
                  ...
                  <td><button className="text-button" onClick={() => toggleUser(user)}>{user.status === 'DISABLED' ? <><CheckCircle2 size={14} /> Enable</> : <><Ban size={14} /> Disable</>}</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
const selectStyle: React.CSSProperties = {
  height: 40, borderRadius: 8, border: '1px solid var(--line)', padding: '0 12px', fontSize: 13, background: '#00000', minWidth: 140,
}
const inputStyle: React.CSSProperties = { height: 40, borderRadius: 8, border: '1px solid var(--line)', padding: '0 12px', fontSize: 13, background: '#00000', width: '100%', marginTop: 4 }
